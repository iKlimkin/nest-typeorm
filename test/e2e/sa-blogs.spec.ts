...
aDescribe(skipSettings.for('SABlogs'))('SABlogsController (e2e)', () => {
  ...
  describe('testing get blogs with paging', () => {
    ...
    it(`/sa/blogs (GET) - shouldn't get blogs without basic auth`, async () => {
      await saBlogTestManager.getSABlogs(null, false, HttpStatus.UNAUTHORIZED);
    });
    it('/sa/blogs (GET) - should receive blogs with sa auth', async () => {
      ...
      const paginationBlogsResultWithQuery = await saBlogTestManager.getSABlogs(
        queryOptions,
      );
      ...
      const blogsWithQueryOptions = paginationBlogsResultWithQueryOptions.items;
  ...
  describe('testing bind blog without owner with user', () => {
    ...
    it('/sa/blogs/:id/bind-with-user/:userId (PUT) - should bind blog with user', async () => {
      ...
      await dataSource.query(`UPDATE blog SET "ownerId" = NULL WHERE id = $1`, [
        blogByFirstToken.id,
      ]);
